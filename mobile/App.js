...
import EmergencyHistoryDetailScreen from './screens/EmergencyHistoryDetailScreen'
...
// Home gets one more entry point than the other self-registered
// screens: past-emergency history (chat included), alongside Account
// and Log out.
function withHomeHeader(title) {
  return ({ navigation }) => ({
    headerShown: true,
    title,
    headerRight: () => (
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        <TouchableOpacity
          ...
          hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
        >
          <Text style={{ color: '#7fe3f2', fontWeight: '600', fontSize: 14 }}>History</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => navigation.navigate('Account')}
          style={{ paddingHorizontal: 8, paddingVertical: 6 }}
      ...
      <SafeAreaProvider>
        <NavigationContainer>
          <Stack.Navigator initialRouteName="Welcome" screenOptions={screenOptions}>
            ...
            <Stack.Screen name="Home" component={UserHomeScreen} options={withHomeHeader('RESQ')} />
            <Stack.Screen name="Account" component={AccountScreen} options={{ headerShown: true, title: 'Account' }} />
            <Stack.Screen name="EmergencyHistory" component={EmergencyHistoryScreen} options={withLogout('Past Emergencies')} />
            <Stack.Screen name="EmergencyHistoryDetail" component={EmergencyHistoryDetailScreen} options={{ headerShown: true, title: 'Emergency Chat' }} />
