...
drop policy if exists "shifts viewable by institution members" on responder_shifts;
drop policy if exists "shifts insert by institution_admin only" on responder_shifts;
drop policy if exists "shifts update by institution_admin only" on responder_shifts;
drop policy if exists "shifts delete by institution_admin only" on responder_shifts;
